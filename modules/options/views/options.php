<?php
class optionsViewTbp extends viewTbp {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = TBP_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. TBP_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. TBP_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			//frameTbp::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameTbp::_()->addJSVar('adminOptionsTbp', 'tbpActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameTbp::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
		$this->assign('mainLink', frameTbp::_()->getModule('supsystic_promo')->getMainLink());
	...
	public function getTabContent() {
		frameTbp::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameTbp::_()->addScript('admin.settings', $this->getModule()->getModPath(). 'js/admin.settings.js');
		frameTbp::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameTbp::_()->getModule('options')->getAll();
		$this->assign('options', $options);
		$this->assign('exportAllSubscribersUrl', uriTbp::mod('subscribe', 'getWpCsvList'));
