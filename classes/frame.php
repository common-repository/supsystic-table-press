...
class frameTbp {
    ...
    public function __construct() {
        $this->_res = toeCreateObjTbp('response', array());
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameTbp();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsTbp::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassTbp($code. strFirstUp(TBP_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameTbp($code);
    ...
    public function init() {
		...
		dispatcherTbp::doAction('afterModulesInit');
		
		modInstallerTbp::checkActivationMessages();
        ...
        register_activation_hook(  TBP_DIR. DS. TBP_MAIN_FILE, array('utilsTbp', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(TBP_DIR. DS. TBP_MAIN_FILE, array('utilsTbp', 'deletePlugin'));
		register_deactivation_hook(TBP_DIR. DS. TBP_MAIN_FILE, array( 'utilsTbp', 'deactivatePlugin' ) );

		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(TBP_LANG_CODE, false, TBP_PLUG_NAME. '/lang');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', TBP_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[TBP_METHODS]) 
                    && !empty($permissions[TBP_METHODS])
                ) {
                    foreach($permissions[TBP_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[TBP_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[TBP_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameTbp::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ TBP_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ TBP_METHODS ][ $action ]))
                            || (!is_array($permissions[ TBP_METHODS ][ $action ]) && $permissions[TBP_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[TBP_USERLEVELS])
                    && !empty($permissions[TBP_USERLEVELS])
                ) {
                    $currentUserPosition = frameTbp::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = TBP_ADMIN;
					}
                    foreach($permissions[TBP_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = TBP_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = TBP_TABLES_DIR) {
        importClassTbp('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableTbp::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameTbp::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriTbp::_($src);
		if(!$ver)
			$ver = TBP_VERSION;
    ...
    /**
     * Add all scripts from _scripts array to wortbpess
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqTbp::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqTbp::getVar('page');
		if(is_admin() && strpos($page, frameTbp::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(TBP_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(TBP_CODE. '_plugin_activation_errors');
	}
}
