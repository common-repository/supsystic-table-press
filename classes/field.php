...
class fieldTbp {
	 ...
	 */
	public $adapt = array('htmlTbp' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterTbp::_($value, $this->adapt['dbFrom'], fieldAdapterTbp::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlTbp', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterTbp::_($this, $this->adapt['html'], fieldAdapterTbp::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameTbp::_()->getModule('optionsTbp')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', TBP_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsTbp'] = $options;
				}
			}
			$htmlContent = htmlTbp::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlTbp::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterTbp::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterTbp::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsTbp');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameTbp::_()->getModule('optionsTbp')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsTbp'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlTbp', $method)) {
			$output .= htmlTbp::$method($name, $params);
			$output .= htmlTbp::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', TBP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', TBP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', TBP_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterTbp::_($id,'getExtraFieldOptions',fieldAdapterTbp::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', TBP_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', TBP_LANG_CODE).':<br />';
					$output .= htmlTbp::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', TBP_LANG_CODE).':<br />';
					$output .= htmlTbp::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterTbp::_($this, $this->adapt['htmlParams'], fieldAdapterTbp::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', TBP_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', TBP_LANG_CODE).':<br />';
				$output .= fieldAdapterTbp::_($params,'getFieldAttributes',  fieldAdapterTbp::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsTbp::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperTbp');
		  $module = $this->checkVarFromParam($param,'moduleTbp');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameTbp::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameTbp::_()->getModule($module)->isExternal())
		   $config_xml = frameTbp::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = TBP_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', TBP_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameTbp::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsTbp'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlTbp::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
