...
class errorsTbp {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqTbp::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $tbpErrors = reqTbp::getVar('tbpErrors');
        if(!empty($tbpErrors)) {
            if(!is_array($tbpErrors)) {
                $tbpErrors = array( $tbpErrors );
            }
            $tbpErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $tbpErrors)));
            if(!empty($tbpErrors)) {
                self::$current = $tbpErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsTbp', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsTbp', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqTbp::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqTbp::clearVar('sesErrors', 'session');
    }
