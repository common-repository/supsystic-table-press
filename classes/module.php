<?php
abstract class moduleTbp extends baseObjectTbp {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsTbp::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsTbp::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return TBP_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return TBP_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importTbp($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameTbp($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassTbp($helper, $this->getModDir(). 'helper.php');
