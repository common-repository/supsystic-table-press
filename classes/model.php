<?php
abstract class modelTbp extends baseObjectTbp {
	...
	public function getModule() {
		return frameTbp::_()->getModule( $this->_code );
	}
	...
	protected function _buildQuery($table = null) {
		if(!$table)
			$table = frameTbp::_()->getTable( $this->_tbl );
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameTbp::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
				return true;
			} else 
				$this->pushError (__('Database error detected', TBP_LANG_CODE));
		} else
			$this->pushError(__('Invalid ID', TBP_LANG_CODE));
		return false;
	}
	...
	public function delete($params = array()) {
		if(frameTbp::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else 
			$this->pushError (__('Database error detected', TBP_LANG_CODE));
		return false;
	...
	public function insert($data) {
		...
		$id = frameTbp::_()->getTable( $this->_tbl)->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameTbp::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	public function updateById($data, $id = 0) {
		...
		if($id) {
			...
			$this->pushError(__('Empty or invalid ID', TBP_LANG_CODE));
		return false;
	}
	public function update($data, $where) {
		$data = $this->_dataSave($data, true);
		if(frameTbp::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameTbp::_()->getTable( $this->_tbl )->getErrors());
		return false;
