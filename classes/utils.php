...
class utilsTbp {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsTbp::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsTbp::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameTbp::_()->getModule($m)) 
                return false;
            if(!is_dir(frameTbp::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameTbp::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqTbp::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse wortbpess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqTbp::getVar('HTTP_REFERER', 'server');
	}
