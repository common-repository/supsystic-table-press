...
class dbTbp {
     ...
     * #__  Wortbpess prefix
     * ^__  Store plugin tables prefix (@see TBP_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, TBP_DB_PREF, $wpdb->prefix. TBP_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampTbp($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, TBP_DATE_DL)) return dateToTimestampTbp($date);
        $arr = explode('-', $date);
        return dateToTimestampTbp($arr[2]. TBP_DATE_DL. $arr[1]. TBP_DATE_DL. $arr[0]);
    }
