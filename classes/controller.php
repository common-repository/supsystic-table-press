...
abstract class controllerTbp {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameTbp::_()->getModule( $this->getCode() );
		$className = '';
		if(importTbp($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameTbp($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importTbp($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameTbp($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(TBP_ADMIN),
				'remove' => array(TBP_ADMIN),
				'restore' => TBP_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - TBP_METHODS or TBP_USERLEVELS
	 */
	...
	public function getModule() {
		return frameTbp::_()->getModule( $this->getCode() );
	}
...
//		$model = $this->getModel('table');
		...
		$page = (int) reqTbp::getVar('page');
		$rowsLimit = (int) reqTbp::getVar('rows');
		$orderBy = reqTbp::getVar('sidx');
		$sortOrder = reqTbp::getVar('sord');

		// Our custom search
		$search = reqTbp::getVar('search');
		...
		// jqGrid search
		$isSearch = reqTbp::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqTbp::getVar('searchField'));
			$searchString = trim(reqTbp::getVar('searchString'));
		...
		$res = dispatcherTbp::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();

	}
	public function removeGroup() {
		$res = new responseTbp();
		if($this->getModel()->removeGroup(reqTbp::getVar('listIds', 'post'))) {
