...
class fieldAdapterTbp {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterTbp', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsTbp'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsTbp::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsTbp::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameTbp::_()->getModule('products')->getCategories();
        $brands = frameTbp::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', TBP_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', TBP_LANG_CODE). '</b><br />
                '. __('Destination', TBP_LANG_CODE). ':'. htmlTbp::checkboxlist('params[dest]', array('optionsTbp' => $listOfDest)). '<br />
                '. __('Country', TBP_LANG_CODE). ':'. htmlTbp::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', TBP_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', TBP_LANG_CODE) : htmlTbp::checkboxlist('params[categories][]', array('optionsTbp' => $cOptions))). '<br />
                    <b>'. __('Brands', TBP_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', TBP_LANG_CODE) : htmlTbp::checkboxlist('params[brands][]', array('optionsTbp' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', TBP_LANG_CODE).': '. htmlTbp::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', TBP_LANG_CODE).': '. htmlTbp::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', TBP_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameTbp::_()->getModule('optionsTbp')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlTbp::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', TBP_LANG_CODE).':<br />';
                $output .= htmlTbp::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', TBP_LANG_CODE).':<br />';
                $output .= htmlTbp::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsTbp']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsTbp'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', TBP_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsTbp'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameTbp::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameTbp::_()->getTable('states')
				->leftJoin( frameTbp::_()->getTable('countries'), 'country_id' )
