...
class langTbp {
	...
	static public function extract($d = array('dir' => '', 'langTbp' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langTbp']) && !empty($d['langTbp']))
			$langDirPath = TBP_LANG_DIR. $d['langTbp']. DS;
		else
			$langDirPath = TBP_LANG_DIR. TBP_WPLANG. DS;
