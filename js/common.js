...
jQuery.fn.serializeAnythingTbp = function(addData, returnArray) {
...
function tbp_str_replace(haystack, needle, replacement) { 
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableTbp(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.tbpExample, .tbpTblHeader').remove();
		var exampleRow = this.table.find('.tbpExample');
		for(var i in list) {
			...
			newRow.removeClass('tbpExample').show();
...
function setCookieTbp(c_name, value, exdays) {
	var exdate = new Date();
 ...
 * @returns {jQuery}
 */
jQuery.fn.tbpZoom = function(level, origin) {
	origin = origin ? origin : 'center center';
	jQuery(this).data('zoom', level);
	return jQuery(this).css({
	/*	'zoom': level	// Didn't worked correctly for mobiles
...
/**
 * Retrive wortbpess attach ID from image, using img classes
...
function randTbp( min, max, returnFloat ) {
