	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('tbpWpTabs')) {
					$this.addClass('tbpWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.tbpTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.tbpTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.tbpTabContent');
					allTabsContent.hide();
