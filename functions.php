 ...
 */
if(!function_exists('dateToTimestampTbp')) {
    function dateToTimestampTbp($date) {
        if(empty($a))
            return false;
        $a = explode(TBP_DATE_DL, $date);
...
if(!function_exists('importTbp')) {
    function importTbp($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassTbp')) {
    function importClassTbp($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), TBP_CODE) !== false) {
					$classFile = preg_replace('/'. TBP_CODE. '/i', '', $classFile);
				}
                $path = TBP_CLASSES_DIR. $classFile. '.php';
			}
            return importTbp($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, TBP_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], TBP_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameTbp')) {
    function toeGetClassNameTbp($class) {
        $className = '';
		if(class_exists($class. strFirstUp(TBP_CODE))) 
			$className = $class. strFirstUp(TBP_CODE);
		else if(class_exists(TBP_CLASS_PREFIX. $class)) 
            $className = TBP_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjTbp')) {
    function toeCreateObjTbp($class, $params) {
        $className = toeGetClassNameTbp($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectTbp')) {
    function redirectTbp($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsTbp')) {
	function prepareParamsTbp(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsTbp::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeTbp($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeTbp')) {
	function prepareFieldCodeTbp($string) {   
